...
class EditPanel extends JFrame implements ActionListener {
		...
		JDateChooser dataStartChoose = new com.toedter.calendar.JDateChooser();
		JDateChooser dataStopChoose = new com.toedter.calendar.JDateChooser();
    	...
    	public EditPanel(JApplet appletRef,DefaultListModel modelRef,JList listRef) {
    		applet=appletRef;
    		model=modelRef;
    		list=listRef;
    		...
    		tTitle.setText(((Wydarzenie) model.getElementAt(index)).getTitle());
    		panel.add(tTitle);
    		...
    		tDescription.setText(((Wydarzenie) model.getElementAt(index)).getDescription());
    	...
    	public void actionPerformed(ActionEvent e)
    	{
    		...
    		String kontrolerDataStart;
    		String kontrolerDataStop;
    		
    		kontrolerDataStart=((JTextField)dataStartChoose.getDateEditor().getUiComponent()).getText();
    		kontrolerDataStop=((JTextField)dataStopChoose.getDateEditor().getUiComponent()).getText();
    			...
    			if((kontrolerDataStart.length()>0) && (kontrolerDataStop.length()>0))
    			{
    				if(data.compareTo(dataStartChoose.getDate())!=1)
    				{
    					if(dataStartChoose.getDate().compareTo(dataStopChoose.getDate())==-1)
    					{
    						...
    						int index=list.getSelectedIndex();
							model.setElementAt(newEvent, index);
    						tTitle.requestFocusInWindow();
    						tTitle.setText("");
    						this.dispose();
    						applet.setEnabled(true);
    						JOptionPane.showMessageDialog(null, "Event has been edited!", "ADDED",JOptionPane.INFORMATION_MESSAGE);
    					}
